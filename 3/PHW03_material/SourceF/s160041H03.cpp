...
int *Find_Euler(Vertex *V, Edge *E, int VN, int EN, int *flag, int *pathN) {
	...
	int degree_test = 0;
	int odd_v = -1;
	for (int v = 0; v < VN; v++) { //Euler cycle 인지, path 인지, 아니면 Eulerian이 아닌지 확인하는 과정.
		if (V[v].degree % 2 != 0) {
			degree_test++; //degree가 홀수인 vertex 개수 확인.
			odd_v = v;  //path의 경우 degree가 홀수인 vertex부터 경로를 시작해야 하므로 홀수 vertex의 index 저장.
		}
	}
	...
	*pathN = 0;
	if (*flag == 0) {  //Euler cycle
		...
		while (!(ST.empty())) {
			vv = ST.top();
			if (V[vv].degree == 0) {
			//현재 방문한 vertex의 degree가 0이면 최종 경로 path에 저장
				path[*pathN] = vv;
				*pathN = *pathN + 1;
				ST.pop();
			}
			else {
	    //degree가 0이 아니면 방문할 vertex 선택하고 방문한 다음, 지나온 edge를 각 adjacent list에서 제거하고 해당 vertex들의 degree -1.
				adj_e = V[vv].S.top();
				edge_i = adj_e->d;
				if (E[edge_i].v1 == vv) { //현재 vertex가 E 배열의 v1으로 접근한 것인지 v2로 접근한 것인지 if문으로 확인.
				...
				pool.freeDBL(adj_e->twin); //twin pointer도 동시에 반환해야 함.
				pool.freeDBL(adj_e);
			}
		}
	}
